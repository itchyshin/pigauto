...
pigauto supports five trait types for phylogenetic imputation:

| Type | R class | Latent encoding | Loss | Example |
|------|---------|-----------------|------|---------|
| Continuous | `numeric` | log (optional) + z-score | MSE | Body mass |
| Count | `integer` | log1p + z-score | MSE | Clutch size |
| Binary | `factor` (2 levels) | 0/1 | BCE with logits | Migratory |
| Categorical | `factor` (>2 levels) | One-hot (K columns) | Cross-entropy | Diet type |
| Ordinal | `ordered` | Integer (0 to K-1) + z-score | MSE | Threat status |

All types are encoded into a continuous latent space.  The gated
ensemble of phylogenetic baseline and GNN correction (the internal
`ResidualPhyloDAE` torch module) operates in this space without
modification.  Type-specific logic appears only at encoding, loss
