...
train_mixed <- function(prep, latent_dim, epochs, nsamp) {
  ...
  count_out     <- matrix(NA_integer_, nsamp, n_obs)
  ...
  for (i in seq_len(nsamp)) {
    ...
    for (j in seq_along(types)) {
      ...
      if (tp %in% c("continuous","proportion")) {
        ...
        mu_phi <- dec(zobs); mu <- as_array(mu_phi$mu); phi <- as_array(mu_phi$phi)
        # NB sampling:
        count_out[i,] <- rnbinom(n_obs, size=phi, mu=mu)
      }
    }
    # categoricals
    for (col in cat_cols) {
...
# 2) Create 5 completed data‐sets:
imps <- mi_create(res, M = 5)

# 3) Inspect the first:
head(imps[[1]]) #(I am not sure how you are enabling CUDA - GPU but it would be great to know - maybe we can have an argument for this?)