# Slice 1b real-data gate: does cross-trait refinement (joint_refine_iter)
...
stopifnot("joint_refine_iter" %in% names(formals(pigauto::impute)))
...
for (rep_id in 1:5) {
  ...
  for (it in ITERS) {
    say(sprintf("rep %d seed %d joint_refine_iter=%d", rep_id, seed, it))
    res <- tryCatch(pigauto::impute(dm, tree, seed = seed, verbose = FALSE,
                                    n_imputations = 1L, joint_refine_iter = it),
