#!/usr/bin/env Rscript
#
# Rscripts/bench_avonet_missingness.R
#
...
#   bench_avonet_missingness.rds    tidy results + timings
#   bench_avonet_missingness.md     human-readable summary
#
# Run via SLURM: see submit_avonet.sh
# Run directly:
...
# -------------------------------------------------------------------------
# Output paths — write to working directory so SLURM $SLURM_SUBMIT_DIR
# captures the results via the post-run tar in submit_avonet.sh
# -------------------------------------------------------------------------

out_rds <- "bench_avonet_missingness.rds"
out_md  <- "bench_avonet_missingness.md"
...
# -------------------------------------------------------------------------
# Load bundled data from installed package
# -------------------------------------------------------------------------
...
# Force predict() onto CPU. At n ~ 10k the MPS attention tensor during
# predict can OOM on Apple Silicon. CC uses CPUs (no MPS), so this is a
# no-op there but is kept for correctness parity with local runs.
