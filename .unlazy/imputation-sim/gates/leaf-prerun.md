...
- [x] G9b: every BACE cell converges by BACE's own assess_convergence() verdict, with the runs setting
  ...
  RESOLVED BY THE CAMPAIGN 2026-09-22: runs = 5 was adopted from the probe; every campaign fit stores BACE's own converged verdict, ESS and drift, and the convergence RATE is reported as a result (computed over every pooled BACE fit; figure recorded in PROGRESS.md and the methods note "Failures and convergence"). Gate ABANDONED as a pass/fail oracle on the pre-run directory; its content is a reported result.
