...
| One recommendation from `suggest_next_observation()` improves realised follow-up imputation over random acquisition | Formula and unit tests; controlled Stage-A harness; and an audited 100-replicate-per-cell Totoro pilot at SHA `beee5df` (800 receipts, zero failed fits) exist. The pilot is mixed and does not meet the registered headline gate; a 1,000-replicate-per-cell campaign remains required before any claim. | experimental | 2026-08-18 |
