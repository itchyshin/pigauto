...
| `multi_impute_analysis()` package implementation passes the analysis-aware fixed-effect gate | Package-level 1,000-replicate-per-cell campaign not yet completed | experimental | 2026-07-11 |
...
The warning-free controls establish that an analysis-aware target is
attainable. The v0.10.0 CRAN gate remains blocked until the implemented
`multi_impute_analysis()` backend passes every pre-specified core cell on a new
frozen package SHA. Legacy conformal-width, Brownian/MC-dropout, PMM, and
posterior-tree draws remain unsupported for downstream inference.
