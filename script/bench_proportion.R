#!/usr/bin/env Rscript
#
# script/bench_proportion.R
#
# Per-type benchmark: PROPORTION traits (0–1 bounded)
# Primary sweep: phylogenetic signal
# Secondary sweep: boundary density (fraction of values near 0 or 1)
#
# Run with
#   cd pigauto && /usr/local/bin/Rscript script/bench_proportion.R
...
# Primary sweep: signal strength
scenarios_primary <- c("signal_0.2", "signal_0.4", "signal_0.6",
                       "signal_0.8", "signal_1.0")

# Secondary sweep: boundary density at signal = 0.6
secondary_scenarios <- c("boundary_0.0", "boundary_0.1", "boundary_0.3")
...
run_one_cell <- function(scenario, frac, rep_id) {
  ...
  if (grepl("^signal_", scenario)) {
    ...
    boundary_frac <- 0.0
  } else if (grepl("^boundary_", scenario)) {
    signal <- 0.6
    boundary_frac <- as.numeric(sub("boundary_", "", scenario))
  } else {
  ...
  df <- simulate_proportion_traits(tree, n_traits,
                                   signal = signal,
  ...
  result <- tryCatch({
    ...
    # Method 1: mean (on logit-z scale)
