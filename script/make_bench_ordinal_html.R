#!/usr/bin/env Rscript
#
# script/make_bench_ordinal_html.R
#
# Build a self-contained HTML report from bench_ordinal.rds.
#
# Run with
#   /usr/local/bin/Rscript script/make_bench_ordinal_html.R

suppressPackageStartupMessages({ })

rds_path <- "script/bench_ordinal.rds"
if (!file.exists(rds_path)) {
  stop("Missing ", rds_path,
       "\nRun script/bench_ordinal.R first.")
}
...
method_order  <- c("median", "baseline", "pigauto")
method_label  <- c(median   = "Median imputation",
                   baseline = "BM baseline (Rphylopars)",
                   pigauto  = "pigauto (BM + GNN)")
method_colour <- c(median   = "#9ca3af",
                   baseline = "#2563eb",
                   pigauto  = "#059669")
...
scenario_label <- setNames(
  gsub("levels_", "", scenarios_primary),
  scenarios_primary
)
scenario_label <- setNames(
  paste0(scenario_label, " levels"),
  scenarios_primary
)
...
chart_rmse <- one_bar_chart("rmse", "RMSE", "RMSE by number of ordinal levels",
                            scenarios_primary, "Number of ordinal levels",
                            higher_is_better = FALSE)
chart_rho  <- one_bar_chart("spearman_rho", "Spearman \u03c1",
                            "Spearman \u03c1 by number of ordinal levels",
                            scenarios_primary, "Number of ordinal levels",
                            higher_is_better = TRUE)

chart_signal <- if (length(secondary_scenarios) > 0) {
  one_bar_chart("rmse", "RMSE",
                "RMSE by phylogenetic signal (5 levels)",
...
for (scen in scenarios_primary) {
  ...
  primary_table_rows <- c(primary_table_rows, sprintf(
    ...
    scenario_label[scen],
    fmt(rmse_vals["median"]),   star("median", best_rmse),
    fmt(rmse_vals["baseline"]), star("baseline", best_rmse),
    fmt(rmse_vals["pigauto"]),  star("pigauto", best_rmse),
    fmt(rho_vals["median"]),    star("median", best_rho),
    fmt(rho_vals["baseline"]),  star("baseline", best_rho),
    fmt(rho_vals["pigauto"]),   star("pigauto", best_rho)
  ))
}
...
rmse_bl_3  <- avg_metric(test_df, "baseline", "levels_3", missing_frac, "rmse")
rmse_pg_3  <- avg_metric(test_df, "pigauto",  "levels_3", missing_frac, "rmse")
rmse_bl_10 <- avg_metric(test_df, "baseline", "levels_10", missing_frac, "rmse")
rmse_pg_10 <- avg_metric(test_df, "pigauto",  "levels_10", missing_frac, "rmse")

verdict1 <- if (is.finite(rmse_bl_3) && is.finite(rmse_pg_3)) {
  sprintf('With 3 ordinal levels the BM baseline achieves RMSE %.3f and pigauto achieves %.3f. The coarse discretisation limits the scope for GNN improvement.',
          rmse_bl_3, rmse_pg_3)
} else 'With few ordinal levels the baseline is already competitive and pigauto matches it.'
...
<head>
<meta charset="utf-8">
<title>pigauto: ordinal-trait benchmark</title>
<style>
...
<h1>Ordinal-trait benchmark: level count sweep</h1>
<p class="meta">
Tree: <code>ape::rtree(', r$n_species, ')</code> &middot;
Traits: ', r$n_traits, ' ordinal per scenario &middot;
Levels: 3 &ndash; 10 &middot;
Methods: median &middot; BM baseline (Rphylopars) &middot; pigauto &middot;
...
<h2>Primary sweep: performance by number of ordinal levels (', as.integer(100 * missing_frac), '% missingness)</h2>
<p class="meta">Average across ', r$n_traits, ' traits and ', r$n_reps, ' replicates. &#9733; marks the best method per scenario.</p>

<table>
<thead>
<tr><th rowspan="2">Levels</th><th colspan="3" style="text-align:center">RMSE (lower is better)</th><th colspan="3" style="text-align:center">Spearman &rho; (higher is better)</th></tr>
<tr><th style="text-align:right">Median</th><th style="text-align:right">BM</th><th style="text-align:right">pigauto</th><th style="text-align:right">Median</th><th style="text-align:right">BM</th><th style="text-align:right">pigauto</th></tr>
</thead>
<tbody>
...
<h2>Secondary sweep: signal strength (5 levels)</h2>
<p class="meta">RMSE by phylogenetic signal at fixed 5 ordinal levels.</p>
<div style="text-align:center;margin:1.5em 0">
', chart_signal, '
</div>
') else '', '
...
<p>Driver: <code>script/bench_ordinal.R</code>. Tree: <code>ape::rtree(', r$n_species, ')</code>. Traits: <code>simulate_ordinal_traits()</code>. Training: ', r$epochs, ' epochs with early stopping. To reproduce: <code>Rscript script/bench_ordinal.R</code>, then <code>Rscript script/make_bench_ordinal_html.R</code>.</p>

<footer>
Source: <code>script/bench_ordinal.R</code> &middot;
Results: <code>script/bench_ordinal.rds</code> &middot;
Report: <code>pkgdown/assets/dev/bench_ordinal.html</code>
</footer>
...
targets <- c("script/bench_ordinal.html",
             "pkgdown/assets/dev/bench_ordinal.html")
for (t in targets) {
