#!/usr/bin/env Rscript
# script/bench_lambda_sweep.R
#
# THE paper's key figure: where exactly does pigauto cross over from
# losing to lm_nonlinear (very low phylo) to winning (moderate phylo)?
#
# DGP fixed at: n=500, ncov=10, beta=1.0, f_type ∈ {nonlinear, interactive}.
# Sweep lambda from 0 to 0.5 in 8 steps.
#
# 48 cells (8 lambda × 2 f_type × 3 reps), ~2-3 hr wall.
...
out_rds <- file.path(HERE, "script", "bench_lambda_sweep.rds")
out_log <- file.path(HERE, "script", "bench_lambda_sweep.log")
...
OBS_PER   <- 5L
N_COVS    <- 10L
LAMBDAS   <- c(0.0, 0.025, 0.05, 0.10, 0.15, 0.20, 0.30, 0.50)
...
log_line("=========================================================")
log_line("LAMBDA SWEEP -- find pigauto's win/loss threshold")
...
m_species_mean <- function(d) {
  ...
  pred[is.na(pred)] <- mean(ddf$y, na.rm=TRUE)
  as.numeric(pred)
}
m_lm <- function(d) {
...
for (i in seq_len(total_cells)) {
  row <- cells[i, ]
  seed <- 23000L + i
  log_line(sprintf("Cell %d/%d: f=%s, lam=%.3f, beta=%.1f, rep=%d (seed=%d)",
