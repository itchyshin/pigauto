#!/usr/bin/env Rscript
#
# script/gnn_evidence_sentinel_prerun.R
#
# GNN evidence sentinel pre-run (G0-approved 2026-08-26).
# Regime-map DGP families F1/F2/F3; paired estimand: held-out blend loss
# minus same-fit decoded baseline (negative = GNN helps).
#
...
#   PIGAUTO_PHYLO_LAMBDA  Pagel tree lambda for DGP (default 0.5)
#
# Output per job: gnn_sentinel_job_<id>.rds

options(warn = 1, stringsAsFactors = FALSE)

CANDIDATE_SHA <- "6fddd79"
MISS_FRAC     <- 0.30
PHYLO_LAMBDA  <- as.numeric(Sys.getenv("PIGAUTO_PHYLO_LAMBDA", "0.5"))
EPOCHS        <- 500L
MISSING_MECH  <- "MCAR"
...
# 12 retained fits: 9 (family × n) timing sentinels + 3 extra F2@n=300 seeds
build_jobs <- function() {
  base <- expand.grid(
    family    = c("F1", "F2", "F3"),
...
job_ids <- if (run_all) seq_len(n_jobs) - 1L else as.integer(job_env)
if (any(is.na(job_ids)) || any(job_ids < 0L) || any(job_ids >= n_jobs)) {
  stop("PIGAUTO_JOB_ID must be in 0..", n_jobs - 1L)
}
...
log_line("=== GNN evidence sentinel pre-run ===")
log_line(sprintf(
  "candidate=%s  driver=%s  host=%s  jobs=%s/%d  phylo_lambda=%.2f",
  CANDIDATE_SHA, driver_sha, hostname,
  paste(job_ids, collapse = ","), n_jobs, PHYLO_LAMBDA
))
log_line(sprintf("miss=%.2f %s  epochs=%d  pkg=%s  out=%s",
                 MISS_FRAC, MISSING_MECH, EPOCHS, pkg_path, out_dir))
...
run_one_job <- function(job_id) {
  row <- jobs[job_id + 1L, , drop = FALSE]
  fam <- row$family
  n_sp <- as.integer(row$n_species)
  seed <- as.integer(row$seed)
  tree_seed <- 20260826L + job_id

  log_line(sprintf("--- job %d  %s  n=%d  seed=%d ---", job_id, fam, n_sp, seed))

  set.seed(tree_seed)
  tree_raw <- ape::rtree(n_sp)
  tree <- transform_tree_pagel_local(tree_raw, PHYLO_LAMBDA)
  ...
  list(
    ...
    r_cal_gnn_mean = mean(gates$r_cal_gnn, na.rm = TRUE),
    r_cal_bm_mean  = mean(gates$r_cal_bm, na.rm = TRUE),
    r_cal_mean_mean = mean(gates$r_cal_mean, na.rm = TRUE),
    gate_open_frac = mean(gates$r_cal_gnn > 0, na.rm = TRUE),
...
for (i in seq_along(job_ids)) {
  jid <- job_ids[[i]]
  out_path <- file.path(out_dir, sprintf("gnn_sentinel_job_%02d.rds", jid))
  saveRDS(results[[i]], out_path)
  log_line("wrote ", out_path)
}

if (length(job_ids) == n_jobs) {
  summary_df <- do.call(rbind, lapply(results, function(r) {
    data.frame(
      ...
      r_cal_gnn_mean = if (is.null(r$r_cal_gnn_mean)) NA_real_ else r$r_cal_gnn_mean,
      r_cal_bm_mean  = if (is.null(r$r_cal_bm_mean)) NA_real_ else r$r_cal_bm_mean,
      r_cal_mean_mean = if (is.null(r$r_cal_mean_mean)) NA_real_ else r$r_cal_mean_mean,
      floor_fired = if (is.null(r$floor_fired)) NA else r$floor_fired,
      error = if (is.null(r$error)) NA_character_ else r$error,
      stringsAsFactors = FALSE
    )
  }))
  saveRDS(summary_df, file.path(out_dir, "gnn_sentinel_prerun_summary.rds"))
  write.csv(summary_df, file.path(out_dir, "gnn_sentinel_prerun_summary.csv"),
            row.names = FALSE)
