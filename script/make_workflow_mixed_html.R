...
#
# Self-contained walkthrough of the full pigauto multiple-imputation
# pipeline on the bundled AVONET 300 mixed-type dataset, written to
# stand alone: a first-time reader can run every code block by
# copy-pasting into an R session, with no external trait-cleanup
# package required.
#
...
<p>This tutorial walks through the full pigauto multiple-imputation
pipeline on a genuinely mixed-type dataset, then runs the three
downstream analysis paths that convert <i>M</i> completed trait
matrices into pooled inference. It is self-contained &mdash; every
code block can be copy-pasted into a fresh R session and uses only
the <code>avonet300</code> data and <code>tree300</code> phylogeny
bundled with pigauto, plus <code>glmmTMB</code>, <code>MCMCglmm</code>,
and <code>BACE</code> for the downstream fits.</p>

<p>If you are completely new to pigauto, the quickest on-ramp is
<code>vignette(&quot;getting-started&quot;, package = &quot;pigauto&quot;)</code>
(live: <a href="articles/getting-started.html">Get started</a>), which
runs a single-shot <code>impute()</code> call before returning here
for the full multiple-imputation workflow.</p>

<p>The tutorial drills into two things the single-call
<code>impute()</code> example does not cover:</p>
...
<p>All three paths share sections&nbsp;1&ndash;4 (data, missing data, tree,
multiple imputation). Only from section&nbsp;5 onwards do they
diverge.</p>
...
<li><a href="#tree">Tree and name matching</a></li>
...
the rest of the tutorial a genuine mixed-type demonstration rather
than a continuous-only pipeline with factor columns carried along
for decoration.</p>
...
<h2 id="tree"><span class="num">3.</span> Tree and name matching</h2>
...
<p>Because <code>avonet300</code> ships pre-aligned with
<code>tree300</code>, the species keys on the rows of <code>df</code>
match the tip labels of <code>tree300</code> exactly. You can confirm
this in one line:</p>

<pre><code>stopifnot(setequal(rownames(df), tree300$tip.label))
#&gt; (silent: all 300 species present on both sides)
...
<div class="tip">
<b>Real datasets rarely match this cleanly.</b> A published phylogeny
and a trait table usually disagree on spelling, taxonomy, and coverage:
one side has <code>Parus_major</code> and the other <code>Cyanistes
caeruleus</code>, or the tree covers 2000 tips and the trait table
only 800. The minimum-viable reconciliation is
<code>keep &lt;- intersect(rownames(dat), tree$tip.label)</code> followed
by <code>dat &lt;- dat[keep, , drop = FALSE]</code> and
<code>tree &lt;- ape::drop.tip(tree, setdiff(tree$tip.label, keep))</code>;
for fuzzy matching, synonym lookup, and genus-level fallbacks you
will want a dedicated tool, but that work sits <i>outside</i> pigauto
and is not re-taught here. From <code>impute()</code>&rsquo;s point
of view the only contract is that <code>rownames(dat)</code> is a
subset of <code>tree$tip.label</code>.
</div>

<h2 id="mi"><span class="num">4.</span> Multiple imputation of all five trait types</h2>
...
the configured uncertainty mechanism (conformal by default, or MC dropout
when requested) for continuous / count / ordinal / proportion traits, and
Bernoulli or categorical sampling for discrete traits.</p>
...
#&gt;   M        : 20 imputations (conformal draws)
#&gt;   Species  : 300
...
continuous trait like <code>Mass</code>, stochastic imputation draws produce
variation across originally-missing cells &mdash; this is the
...
<h2 id="patha"><span class="num">5.</span> Path A &mdash; pigauto + <code>glmmTMB</code> + Rubin&rsquo;s rules</h2>
...
section&nbsp;4 above). This is exactly the regime in which Path&nbsp;B
...
<h2 id="pathb"><span class="num">6.</span> Path B &mdash; pigauto + <code>MCMCglmm</code> + posterior concatenation</h2>
...
matrices treats pigauto&rsquo;s imputation draws as approximate
...
<h2 id="pathc"><span class="num">7.</span> Path C &mdash; BACE integrated chained-equation MCMC</h2>
...
callout in section&nbsp;6: the imputation draws are treated as
...
targets <- c(out, "pkgdown/assets/pigauto_workflow_mixed.html")
for (t in targets) {
