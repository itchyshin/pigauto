#!/usr/bin/env Rscript
#
# script/bench_external_comparators.R
#
...
# 5 reps, mask seeds 2026 + rep. Reported as mean +/- MCSE (sd/sqrt(n)).
#
...
#
# Output:
#   script/bench_external_comparators.rds
#   script/bench_external_comparators.md
#
...
MISS_FRAC <- 0.30
MASK_SEED_BASE <- 2026L
N_REPS <- {
  v <- Sys.getenv("PIGAUTO_N_REPS", unset = "")
...
for (ds_name in names(DATASETS)) {
  ...
  for (rep_id in seq_len(N_REPS)) {
    mask_seed <- MASK_SEED_BASE + rep_id
...
md <- c(
  ...
  sprintf("- Missingness: %.0f%% MCAR on observed cells, %d reps, mask seeds %d..%d.",
          MISS_FRAC * 100, N_REPS, MASK_SEED_BASE + 1L, MASK_SEED_BASE + N_REPS),
