#!/usr/bin/env Rscript
#
# F2 @ λ_DGP=1 confirmatory arm (prereg §3.4).
# 5 G4-explore-pass cells × 60 paired seeds = 300 fits.
# Primary estimand: lambda_mode = "fixed_1".
#
# Cells (F2, phylo_lambda=1.0, MCAR):
#   n=300  miss 10/30/50%
#   n=1000 miss 10/30%  (n=1000 @ 50% failed G4 rel-improve — excluded)
#
# Fresh seeds (60000-block) — separate from 30-seed explore arm.
#
# Env:
#   PIGAUTO_PKG_PATH, PIGAUTO_OUT_DIR, PIGAUTO_JOB_ID,
#   PIGAUTO_SKIP_EXISTING, PIGAUTO_SMOKE
...
N_REPS        <- 60L
SEED_BASE     <- 60000L
TREE_SEED_BASE <- 20260827L

CONFIRM_CELLS <- data.frame(
  confirm_cell_id = 0:4L,
  family = "F2",
  n_species = c(300L, 300L, 300L, 1000L, 1000L),
  phylo_lambda = 1.0,
  miss_frac = c(0.10, 0.30, 0.50, 0.10, 0.30),
  explore_cell_id = c(42L, 43L, 44L, 51L, 52L),
...
out_dir <- Sys.getenv("PIGAUTO_OUT_DIR", unset = file.path(getwd(), "results_confirm"))
...
build_jobs <- function() {
  ...
  jobs <- merge(CONFIRM_CELLS, reps, by = NULL)
  jobs$job_id <- seq_len(nrow(jobs)) - 1L
  jobs$seed <- SEED_BASE + jobs$confirm_cell_id * 100L + jobs$rep
  jobs[, c("job_id", "confirm_cell_id", "explore_cell_id", "family",
           "n_species", "phylo_lambda", "miss_frac", "rep", "seed")]
}
...
log_line("=== F2 @ lambda=1 confirmatory arm ===")
log_line(sprintf(
...
log_line(sprintf("cells=%d  reps=%d  out=%s", nrow(CONFIRM_CELLS), N_REPS, out_dir))
...
result_stub <- function(row, fit_ok, error = NA_character_, fit_sec = NA_real_,
  ...
  list(
    campaign = "f2_confirm",
    job_id = row$job_id,
    confirm_cell_id = row$confirm_cell_id,
    explore_cell_id = row$explore_cell_id,
    rep = row$rep,
    family = row$family,
    n_species = row$n_species,
    seed = row$seed,
    tree_seed = TREE_SEED_BASE + row$job_id,
...
run_one_job <- function(job_id) {
  ...
  out_path <- file.path(out_dir, sprintf("gnn_confirm_job_%04d.rds", job_id))
  ...
  miss_frac <- row$miss_frac
  tree_seed <- TREE_SEED_BASE + job_id

  log_line(sprintf(
    "job %d  confirm_cell=%d  explore_cell=%d  n=%d  miss=%.0f%%  rep=%d  seed=%d",
    job_id, row$confirm_cell_id, row$explore_cell_id,
    n_sp, miss_frac * 100, row$rep, seed
  ))

  set.seed(tree_seed)
  tree_raw <- ape::rtree(n_sp)
  tree <- transform_tree_pagel_local(tree_raw, row$phylo_lambda)

  Z <- simulate_latents(tree, seed = seed + 1000L)
  df_full <- family_to_traits(Z, "F2")
...
for (i in seq_along(job_ids)) {
  jid <- job_ids[[i]]
  out_path <- file.path(out_dir, sprintf("gnn_confirm_job_%04d.rds", jid))
...
if (length(job_ids) == nrow(build_jobs()) && !identical(Sys.getenv("PIGAUTO_SMOKE", "0"), "1")) {
  fs <- sort(Sys.glob(file.path(out_dir, "gnn_confirm_job_*.rds")))
  ...
  saveRDS(summary_df, file.path(out_dir, "gnn_confirm_summary.rds"))
  write.csv(summary_df, file.path(out_dir, "gnn_confirm_summary.csv"), row.names = FALSE)
