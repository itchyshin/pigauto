...
`pigauto` is an R package (version 0.3.0) for phylogenetic trait imputation. It fits a Residual Phylogenetic Denoising Autoencoder (ResidualPhyloDAE) that blends a Brownian-motion baseline with a graph neural network correction. See `README.md` for the user-facing API; this file documents the internals.
